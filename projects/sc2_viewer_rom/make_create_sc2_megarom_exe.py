"""Utilities to bundle create_sc2_megarom into a single-file executable with Nuitka."""
...
def build_executable(output_dir: Path) -> None:
    """Compile ``create_sc2_megarom.py`` into a one-file executable via Nuitka."""
    script_dir = Path(__file__).resolve().parent
    target = script_dir / "src" / "create_sc2_megarom.py"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Build a standalone executable for create_sc2_megarom using Nuitka",
    )
